...
# 数据源配置
OFFICIAL_URL = CONFIG.get("official_url", "https://bet.hkjc.com/contentserver/jcbw/cmc/last30draw.json")
THIRD_PARTY_URLS = CONFIG.get("third_party_urls", [
    "https://marksix6.net/index.php?api=1",
    "https://lottolyzer.com/history/hong-kong/mark-six/page/1/per-page/50/summary-view",
])
THIRD_PARTY_MAX_PAGES = CONFIG.get("third_party_max_pages", 60)
...
SUM_TARGET = CONFIG.get("sum_target", (115, 185))  # 历史常见和值区间
...
def _ensure_columns(conn: sqlite3.Connection) -> None:
    ...
    for col in desired - existing:
        conn.execute(f"ALTER TABLE draws ADD COLUMN {col} INTEGER" if col != "zodiac_json" else f"ALTER TABLE draws ADD COLUMN {col} TEXT")
...
def parse_csv_text(text: str) -> List[DrawRecord]:
    ...
    for row in reader:
        ...
        if not issue or not nums_str:
            continue
        try:
            nums = [int(x) for x in nums_str.replace("，", ",").split(",") if x.strip().isdigit()]
            special = int(special_str)
        except:
            continue
        if len(nums) >= 6 and 1 <= special <= 49:
            records.append(DrawRecord(issue_no=issue.strip(), draw_date=date.strip(), numbers=nums[:6], special_number=special))
    return records


def fetch_all_sources() -> List[DrawRecord]:
    ...
    for url in THIRD_PARTY_URLS:
        ...
        elif "lottolyzer" in url:
            records = parse_lottolyzer_html(text)
    ...
    for issue, variants in all_records.items():
        if len(variants) >= 2:
            ...
            if counts[most_common] >= 2:
                base = variants[0]
                final_records.append(DrawRecord(
                    issue_no=issue,
    ...
    logger.info(f"Cross-validation: total {len(final_records)} records from {len(all_records)} issues")
...
def parse_lottolyzer_html(html: str) -> List[DrawRecord]:
    ...
    pattern = re.compile(r"(?P<issue>\d{2}/\d{3})\s+(?P<date>\d{4}-\d{2}-\d{2})\s+(?P<nums>[\d,\s]+?)\s+(?P<extra>\d{1,2})\b")
    records = []
    for m in pattern.finditer(text):
        ...
        if len(nums) >= 6:
            records.append(DrawRecord(issue_no=issue, draw_date=date, numbers=nums[:6], special_number=extra))
    return records
...
def calculate_pair_lift(draws: List[List[int]]) -> Dict[Tuple[int, int], float]:
    ...
    for (a, b), cnt in pair_count.items():
        expected = (single_count[a] / total_draws) * (single_count[b] / total_draws) * total_draws
        if expected > 0:
...
def monte_carlo_pick(scores: Dict[int, float], pair_lift: Dict[Tuple[int, int], float],
                     trials: int = MONTE_CARLO_TRIALS) -> List[int]:
    """蒙特卡洛模拟寻找最优6码组合"""
    ...
    if not best_combo:
        best_combo = [n for n, _ in sorted(scores.items(), key=lambda x: x[1], reverse=True)[:6]]
    return best_combo
...
# -------------------- 策略核心 --------------------
def generate_strategy_score(draws: List[List[int]], specials: List[int], strategy: str,
                            pair_lift: Dict[Tuple[int, int], float]) -> StrategyScore:
    cfg = STRATEGY_CONFIGS.get(strategy, STRATEGY_CONFIGS["balanced"])
    freq = {n: sum(1 for d in draw for n in d) for d in draws for n in ALL_NUMBERS}
    omit = {}
    ...
    freq_n = norm(freq); omit_n = norm({n: 1.0/(omit[n]+1) for n in ALL_NUMBERS}); mom_n = norm(mom)
    scores = {n: freq_n[n]*cfg["w_freq"] + omit_n[n]*cfg["w_omit"] + mom_n[n]*cfg["w_mom"] for n in ALL_NUMBERS}

    main_picks = monte_carlo_pick(scores, pair_lift) if strategy != "ensemble" else ensemble_vote(draws, specials, pair_lift).main_picks
...
def ensemble_vote(draws: List[List[int]], specials: List[int], pair_lift: Dict) -> StrategyScore:
    scores_list = []
    ...
    while sp in main_picks:
        sp = (sp % 49) + 1
    return StrategyScore(main_picks, sp, sum(norm_votes[n] for n in main_picks)/6, norm_votes)
...
def cmd_sync(args):
    ...
    for (a,b), lift in pair_lift.items():
        conn.execute("INSERT INTO pair_affinity (num1, num2, lift, updated_at) VALUES (?,?,?,?)",
                     (a, b, lift, utc_now()))
    conn.commit()
    # 增量回测
...
def cmd_predict(args):
    ...
    specials = [r[1] for r in conn.execute("SELECT special_number FROM draws ORDER BY draw_date DESC LIMIT 200").fetchall()]
    pair_lift = calculate_pair_lift(draws)
...
def cmd_show(args):
    conn = connect_db(args.db)
    init_db(conn)
    # 原样保留现有输出格式，此处仅展示后台计算的新增统计信息
    # 实际运行时会先打印原有表格，然后增加新统计
    print("===== 专业版增强统计 =====")
    stats = conn.execute("SELECT * FROM backtest_stats ORDER BY sharpe_ratio DESC").fetchall()
    for s in stats:
        print(f"{s['strategy']}: 夏普={s['sharpe_ratio']:.2f} 平均命中={s['avg_hit']:.2f} ≥2码率={s['hit2_rate']*100:.1f}%")
    conn.close()
